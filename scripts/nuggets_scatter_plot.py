...
from bokeh.models import ColumnDataSource, CategoricalColorMapper, NumeralTickFormatter, HoverTool
...
pct_fig = figure(title='2Pt FG % vs 3Pt FG %',
				 plot_height=400,
				 plot_width=400,
				 tools=tool_list,
				 x_axis_label='2Pt FG',
...
# Circle markers
pct_fig.circle(x='team2P%', y='team3P%', source=nugget_stats, size=12, color='black')
# X and Y gradient
pct_fig.xaxis[0].formatter = NumeralTickFormatter(format='00.0%')
pct_fig.yaxis[0].formatter = NumeralTickFormatter(format='00.0%')

# Comparison figure
total_fig = figure(title='Team Points vs Opponent Points',
				   plot_height=400,
				   plot_width=400,
				   tools=tool_list,
